...
DuplicationManager::DuplicationManager()
    : m_device(nullptr), m_device_context(nullptr),
      m_output_duplication(nullptr), m_back_texture(nullptr),
      m_input_layout(nullptr), m_vertex_shader(nullptr),
      m_vertex_buffer(nullptr), m_y_render_texture(nullptr),
      m_y_staging_texture(nullptr), m_y_rtv(nullptr), m_y_pixel_shader(nullptr),
      m_uv_render_texture(nullptr), m_uv_staging_texture(nullptr),
      m_uv_rtv(nullptr), m_uv_pixel_shader(nullptr),
...
bool DuplicationManager::Init(int display_index) {
  ...
  hr = D3D11CreateDevice(display_output->adapter, D3D_DRIVER_TYPE_UNKNOWN,
                         nullptr, D3D11_CREATE_DEVICE_BGRA_SUPPORT,
                         feature_levels, feature_levels_size, D3D11_SDK_VERSION,
                         &m_device, &feature_level, &m_device_context);
  ...
  ffi_log(FFI_LOG_INFO, "D3D11CreateDevice succeeded, feature level:%s",
  ...
  hr = display_output->output->QueryInterface(
      __uuidof(IDXGIOutput1), reinterpret_cast<void **>(&dxgi_output));
  if (FAILED(hr)) {
...
void DuplicationManager::CaptureFrame(void *tx, capture_callback cb) {
  ...
  m_device_context->Map(m_y_staging_texture, 0, D3D11_MAP_READ, 0,
                        &y_mapped_resource);
  m_device_context->Map(m_uv_staging_texture, 0, D3D11_MAP_READ, 0,
                        &uv_mapped_resource);
  ...
  cb(tx, width, height, y_mapped_resource.RowPitch,
     (uint8_t *)y_mapped_resource.pData, uv_mapped_resource.RowPitch,
...
bool DuplicationManager::init_shaders() {
  ...
  m_device_context->IASetVertexBuffers(0, 1, &m_vertex_buffer, &vertex_stride,
                                       &vertex_offset);
  ...
  hr = m_device->CreateInputLayout(input_element_desc, input_element_desc_size,
                                   g_VS, g_VS_SIZE, &m_input_layout);
  if (FAILED(hr)) {
  ...
  hr = m_device->CreatePixelShader(g_PS_Y, pixel_shader_y_size, nullptr,
                                   &m_y_pixel_shader);
  if (FAILED(hr)) {
  ...
  hr = m_device->CreatePixelShader(g_PS_UV, pixel_shader_uv_size, nullptr,
                                   &m_uv_pixel_shader);
  if (FAILED(hr)) {
...
bool DuplicationManager::init_back_texture() {
  ...
  hr = m_device->CreateTexture2D(&back_texture_desc, nullptr,
                                 &m_y_render_texture);
  ...
  set_viewport(&m_y_viewport, back_texture_desc.Width,
  ...
  set_viewport(&m_uv_viewport, back_texture_desc.Width,
               back_texture_desc.Height);
  return make_rtv(&m_uv_rtv, m_uv_render_texture);
}

void DuplicationManager::set_viewport(D3D11_VIEWPORT *viewport, size_t width,
                                      size_t height) {
  viewport->TopLeftX = 0;
  viewport->TopLeftY = 0;
...
bool DuplicationManager::process_frame() {
  ...
  HRESULT hr = m_device->CreateShaderResourceView(
      m_back_texture, &shader_resource_view_desc, &shader_resource_view);
  if (FAILED(hr)) {
