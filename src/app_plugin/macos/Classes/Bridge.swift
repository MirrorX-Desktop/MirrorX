...
//func dispatchFrame(
//    textureID: Int64, cvPixelBufferRef: Unmanaged<CVPixelBufferRef>
//) -> CBool {
...
//
//        print("notify")
//        videoTexture.notifiyTextureUpdate(
//            textureID: textureID, width: width, height: height, isFullColorRange: isFullColorRange,
//            yPlaneBufferAddress: yPlaneBufferAddress, yPlaneStride: yPlaneStride,
//            uvPlaneBufferAddress: uvPlaneBufferAddress, uvPlaneStride: uvPlaneStride, dts: dts, pts: pts)
//    }
//
//    return true
//}

let update_frame_callback:@convention(c)(Int64,UnsafeMutableRawPointer) -> Void = { (a,b) in
    makesureGlobalVideoTexturesLockerInitialized()
    
    pthread_rwlock_rdlock(&globalVideoTexturesLocker)
    let videoTexture = globalVideoTextures[a]
    pthread_rwlock_unlock(&globalVideoTexturesLocker)
    
    let p = Unmanaged<CVPixelBuffer>.fromOpaque(b)
    
    videoTexture?.notifiyTextureUpdate(textureID:a,pixelBuffer: p)
}
