...
void UpdateFrameCallback(int64_t texture_id, VideoTexture* video_texture,
	uint8_t* new_frame_ptr) {
...
namespace texture_render {
	...
	void TextureRenderPlugin::HandleMethodCall(
		...
		if (method_call.method_name().compare("register_texture") == 0) {
			...
			if (texture_id == -1) {
				result->Error("video_texture::RegisterTexture failed");
				return;
			}
