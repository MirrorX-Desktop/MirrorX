...
fn open_packet(
    ...
    handle_message(active_device_id, passive_device_id, message, message_tx);
...
fn handle_message(
    ...
    match message {
        EndPointMessage::Error => {
            TOKIO_RUNTIME.spawn(handle_error(active_device_id, passive_device_id));
        }
        EndPointMessage::NegotiateVisitDesktopParamsRequest(req) => {
            TOKIO_RUNTIME.spawn(handle_negotiate_visit_desktop_params_request(
                active_device_id,
                passive_device_id,
                req,
                message_tx,
        ...
        EndPointMessage::NegotiateVisitDesktopParamsResponse(resp) => {
            TOKIO_RUNTIME.spawn(handle_negotiate_visit_desktop_params_response(
                active_device_id,
                passive_device_id,
                resp,
            ));
        ...
        EndPointMessage::NegotiateSelectMonitorRequest(req) => {
            TOKIO_RUNTIME.spawn(handle_negotiate_select_monitor_request(
                active_device_id,
                passive_device_id,
                req,
                message_tx,
        ...
        EndPointMessage::NegotiateSelectMonitorResponse(resp) => {
            TOKIO_RUNTIME.spawn(handle_negotiate_select_monitor_response(
                active_device_id,
                passive_device_id,
                resp,
            ));
        ...
        EndPointMessage::NegotiateFinishedRequest(req) => {
            TOKIO_RUNTIME.spawn(handle_negotiate_finished_request(
                active_device_id,
                passive_device_id,
                req,
                message_tx,
        ...
        EndPointMessage::VideoFrame(video_frame) => {
            handle_video_frame(active_device_id, passive_device_id, video_frame);
        }
        EndPointMessage::AudioFrame(audio_frame) => {
            handle_audio_frame(active_device_id, passive_device_id, audio_frame);
        }
        EndPointMessage::Input(input_event) => {
            TOKIO_RUNTIME.spawn(handle_input(
                active_device_id,
                passive_device_id,
                input_event,
            ));
