...
impl DomainRepository {
    ...
    pub fn ensure_table(&self) -> CoreResult<()> {
        ...
        const COMMAND_CREATE_TABLE: &str = r"
        ...
        conn.execute(COMMAND_CREATE_TABLE, [])?;

        const COMMAND_INSERT_DEFAULT_DOMAIN: &str = r"
        INSERT
        INTO domains(name,
                     addr,
                     is_primary,
        ...
        WHERE NOT EXISTS(
                SELECT 1
                FROM domains
                WHERE name = 'MirrorX.cloud')";

        let password = utility::rand::generate_random_password();
        let finger_print = utility::rand::generate_device_finger_print();

        conn.execute(
            COMMAND_INSERT_DEFAULT_DOMAIN,
            params![
                "MirrorX.cloud",
                "mirrorx.cloud:28000",
                true,
